...
const maximumNumber = numbersEx2.reduce(
  (maximum, number) => (number > maximum ? number : maximum),
  -Infinity
);
console.log('Ex. 2 result', maximumNumber);

// Exercise 3: Explain the code
// Count the occurrences of each element in the given array and return an object with the counts.
// The accumulator starts as an empty object. For each letter, the code either
// increases its existing count or adds the letter with a count of 1.
...
const sumEx4 = numbersEx4.reduce((sum, number) => sum + number, 0);
const averageEx4 = sumEx4 / numbersEx4.length;
console.log('Ex. 4 result', averageEx4);


// Exercise 5: Explain the code below
// Count the number of unique elements in the array.
// The accumulator is an array. A number is added only if it is not already
// present; after reduce finishes, .length gives the unique-number count.
